...
function generateShoppingList(days = 7, startDate = selectedDate) {
  const counts = {};
  const householdSize = state.settings.householdSize || 1;
  for (let i = 0; i < days; i++) {
    ...
    ["breakfast", "lunch", "dinner", "snack"].forEach((slot) => {
      ...
      counts[id] = (counts[id] || 0) + householdSize;
    });
  }
...
function renderMeals() {
  ...
  const hh = state.settings.householdSize || 1;
  html += `<div class="card"><h3>Shopping list — next 7 days${hh > 1 ? ` (for ${hh} people)` : ""}</h3><ul class="shopping-list">`;
...
function renderSettings() {
  ...
  el("settings").innerHTML = `<div class="card">
    ...
    <form id="settingsForm" class="settings-form">
      ...
      <label>Cooking for how many people (affects the Meals tab shopping list only)
        <input type="number" id="householdSize" value="${s.householdSize || 1}" min="1" max="8" />
      </label>
  ...
  document.getElementById("settingsForm").addEventListener("submit", (e) => {
    ...
    state.settings.householdSize = parseInt(document.getElementById("householdSize").value, 10) || 1;
    saveState();
    renderAll();
    switchTab("dashboard");
  });
