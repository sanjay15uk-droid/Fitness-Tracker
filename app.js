...
function dayEntry(key) {
  if (!state.log[key]) {
    state.log[key] = { cardio: [], gym: { completedExerciseIds: [], weights: {}, sessionComplete: false }, meals: { breakfast: false, lunch: false, dinner: false, snack: false }, customFood: [] };
  }
...
function planDayIndex(d) {
  return (d.getDay() + 6) % 7; // 0 = Mon ... 6 = Sun, matches DEFAULT_WEEK_PLAN order
}
function slotsEatenKcal(entry, dateForPlan = selectedDate) {
  const dayPlan = DEFAULT_WEEK_PLAN[planDayIndex(dateForPlan)];
  const slotIds = [dayPlan.breakfast, dayPlan.lunch, dayPlan.dinner, dayPlan.snack];
  const slotNames = ["breakfast", "lunch", "dinner", "snack"];
  const planned = slotNames.reduce((sum, slot, i) => sum + (entry.meals[slot] ? findMeal(slotIds[i]).meal.kcal : 0), 0);
...
function renderMeals() {
  ...
  const entry = dayEntry(key);
  const targets = calcTargetsForDay(key);
  const dayPlan = DEFAULT_WEEK_PLAN[planDayIndex(selectedDate)];
  const slots = [
    { slot: "breakfast", id: dayPlan.breakfast },
    { slot: "lunch", id: dayPlan.lunch },
    { slot: "dinner", id: dayPlan.dinner },
    { slot: "snack", id: dayPlan.snack },
  ];
  ...
  let html = `<div class="card">
    ...
    <p class="hint">Tick off each meal as you eat it. Based on the default plan below — swap freely, this is just tracking whether you ate, not forcing the exact recipe.</p>
