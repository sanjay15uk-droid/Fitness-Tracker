...
function dayEntry(key) {
  if (!state.log[key]) {
    state.log[key] = { cardio: [], gym: { completedExerciseIds: [], weights: {}, sessionComplete: false }, meals: { breakfast: false, lunch: false, dinner: false, snack: false } };
  }
...
function renderMeals() {
  ...
  const eatenKcal = slots.reduce((sum, s) => sum + (entry.meals[s.slot] ? findMeal(s.id).meal.kcal : 0), 0);
