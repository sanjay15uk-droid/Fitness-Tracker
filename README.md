# The Pain Cave — Training Tracker
...
## One-tap cardio logging (Apple Shortcuts)

The app recognises a special link: opening `https://yourusername.github.io/reponame/?logcardio=run&minutes=30` (or `padel` instead of `run`) logs that session instantly with no typing, then cleans the link so refreshing doesn't double-log it. Optionally add `&kcal=280` if you want to pass a Watch calorie figure too.

To wire this to a Shortcut:
1. Open the **Shortcuts** app → **+** → new shortcut.
2. Add **Ask for Input** (Number) — prompt "Minutes?".
3. Add **Text** action, set it to `https://yourusername.github.io/reponame/?logcardio=run&minutes=` then insert the number from step 2 at the end (tap the blue variable chip).
4. Add **Open URLs** using that text.
5. Name it "Log run" and add it to your home screen, or trigger it from a Personal Automation for "Workout Ends" on your Watch.

## Home screen widget (what's scheduled today)

A true live-updating widget isn't possible for a website — Apple only allows that for native apps, and your actual logged data lives inside your phone's browser, which nothing outside it can read. What you can get instead: a one-tap Shortcuts widget that tells you today's scheduled session using nothing but the calendar (no network, so it can't break).

Your current weekly template (edit to match if you change `WEEK_TEMPLATE` in `data.js`):
Mon → Push · Tue → Pull · Wed → Legs · Thu → Rest · Fri → Push · Sat → Rest · Sun → Rest

To build it:
1. Shortcuts app → **+** → new shortcut.
2. Add **Current Date**, then **Format Date** set to "Weekday Name" to get e.g. "Monday".
3. Add an **If** block for each day, output text like "Push day" / "Pull day" / "Legs day" / "Rest day" to match the template above.
4. Add **Show Result** with that text.
5. Name it "Today's session", then long-press your home screen → **+** → search **Shortcuts** → add the widget → choose this shortcut.

Tapping the widget shows today's session type in a couple of seconds — it won't reflect swaps or completions (those live in the app only), but it tells you whether to expect a gym day before you've even opened your phone properly.
